...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;background-color:${EMAIL_BRAND.white};border-radius:12px 12px 0 0;overflow:hidden;">
  <tr>
    <td style="background-color:${EMAIL_BRAND.blue};padding:28px 32px;text-align:center;">
      ...
      <p style="margin:10px 0 0;font-family:${EMAIL_FONTS.body};font-size:13px;color:rgba(255,255,255,0.88);">${escapeHtml(COMPANY_TAGLINE)}</p>
    </td>
  </tr>
  <tr>
...
export function buildAdminLeadEmailHtml(leadData) {
  ...
  const badge = n.isRegistration ? "BOOTCAMP REGISTRATION" : "NEW LEAD";
...
${brandFooter({ includeUnsubscribe: true })}`;
  ...
  return emailDocument({
    title: n.isRegistration ? "Bootcamp registration" : "New lead",
    preheader: headline,
    bodyHtml: body,
  });
}
...
export function buildAdminLeadEmailText(leadData) {
  const n = normalizeLead(leadData);
  const lines = [
    n.isRegistration ? "BOOTCAMP REGISTRATION" : "NEW LEAD",
    "—".repeat(40),
    `Name: ${n.name}`,
    `Email: ${n.email}`,
    `Phone: ${n.phone}`,
...
export function buildCustomerLeadEmailHtml(leadData) {
  ...
  const intro = n.isRegistration
    ? `Thank you for registering for the <strong style="color:${EMAIL_BRAND.blue};">Build With Innocent Coding Bootcamp</strong>. Your application is confirmed.`
    : `Thank you for reaching out to <strong style="color:${EMAIL_BRAND.blue};">Build With Innocent</strong>. We have received your consultation request.`;

  const summaryExtra = n.isRegistration
    ? `
...
<tr><td colspan="2" style="padding:6px 0;font-size:14px;color:${EMAIL_BRAND.text};background:${EMAIL_BRAND.surface};border-radius:8px;padding:12px;border-left:3px solid ${EMAIL_BRAND.green};">${escapeHtml(n.goals || "—")}</td></tr>`
    : `
<tr><td style="padding:10px 0;font-size:13px;color:${EMAIL_BRAND.textMuted};">WhatsApp</td>
<td align="right" style="padding:10px 0;font-size:14px;color:${EMAIL_BRAND.text};">${phone}</td></tr>`;
  ...
  <tr>
    <td style="padding:0 32px 32px;text-align:center;">
      ...
      <p style="margin:20px 0 0;font-size:14px;color:${EMAIL_BRAND.textMuted};">
        Explore our work: <a href="${SITE_URL}" style="color:${EMAIL_BRAND.blue};font-weight:700;text-decoration:none;">buildwithinnocent.com</a>
      </p>
    </td>
  </tr>
</table>
${brandFooter({ includeUnsubscribe: true })}`;
  ...
  return emailDocument({
    ...
    preheader: n.isRegistration ? "Bootcamp registration confirmed" : "We received your message",
    bodyHtml: body,
  });
}
...
export function buildCustomerLeadEmailText(leadData) {
  ...
  const lines = [
    ...
    n.isRegistration
      ...
      : "Thank you for reaching out to Build With Innocent.",
    "",
